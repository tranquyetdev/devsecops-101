...
```bash
stock-trading$ sam logs -n StockCheckerFunction --stack-name stock-trading --tail
```
...
```bash
stock-trading$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name stock-trading
```
